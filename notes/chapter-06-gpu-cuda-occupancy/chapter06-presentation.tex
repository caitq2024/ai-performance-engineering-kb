...
\begin{frame}{Roadmap: Five Parts, One Golden Rule}
  \begin{enumerate}
    \item \kw{GPU architecture} (slides 4--16) --- threads/blocks/grids, warps \& SIMT,
          inside the SM, divergence, hardware limits, compatibility
  ...
  \begin{columns}[c]
    ...
    \begin{itemize}
      ...
      \item The GPU's answer is not faster copies but \kw{massive parallelism}:
            when one batch of threads waits on memory, another batch runs.
...
\begin{frame}{Warps and SIMT: 32 Threads in Lockstep}
  ...
  \end{columns}
  \medskip
  \pe{a warp is a rowing crew of 32 --- same stroke, same beat, or the boat waits.}
\end{frame}
