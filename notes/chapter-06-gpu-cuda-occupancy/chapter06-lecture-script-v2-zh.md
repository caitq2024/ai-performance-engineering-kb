# 第六章讲稿 v2（中英对照，逐页对应 chapter06-presentation-v2.pdf，共 52 页 / 约 1 小时）

> **v2 变化**：页码与 v1 完全一致；改的是叙事——第 2/3 页重建问题意识、6 个标题改成结论句、比喻统一为"工厂/车间/小组"体系、occupancy 误区提前到第 15 页、每个 Part 之间加转场（本稿中标 🗣）。

> **用法**：左边开着 `chapter06-presentation-v2.pdf`（52 页版），右边看这份讲稿。每节对应一页，包含：
...
> | Part I GPU 架构（含 3 页比喻图） | 4–19 | 15 分钟 |
> | Part II CUDA 编程 | 20–27 | 10 分钟 |
> | Part III 内存层级 | 28–36 | 11 分钟 |
> | Part IV Occupancy 实战 | 37–45 | 11 分钟 |
> | Part V 正确性与 Roofline | 46–49 | 6 分钟 |
> | 收尾（要点/结论/参考） | 50–52 | 3 分钟 |
>
> ⭐ 52 页不能平均用力。**核心重讲页**（每页约 90 秒）：5、6、9、10、15、20、24、28、37、38、40、42、43、47、49、51；其余页面是支撑这些核心页的证据，30–40 秒带过、不逐条念。赶时间时第 7、19、25、27、33 页各压到 30 秒；比喻三连页（8–10）可并成一页快讲。
...
## 第 2 页｜This Talk in Three Questions（本讲三问）⭐

🎤 这一章细节非常多，为了让大家全程不迷路，我把整章压缩成**三个问题**，今天讲的每一页都挂在这三个问题下面：
**问题一：GPU 怎样把大规模并行转化成吞吐量？**——成千上万的线程被组织成 warp 和 block、铺满各个 SM。这带来**两层收益**：第一，同时做更多计算；第二，某个 warp 停顿时，还有其他 warp 可以继续执行——**后者才叫 latency hiding（延迟隐藏）**，它是并行有效的一个机制，不是 GPU 并行的全部优点。这是 Part I。
**问题二：CUDA 怎么制造足够多的并行工作？**——grid、block 和启动参数决定 GPU 能拿到多少活。这是 Part II。
**问题三：GPU 忙起来之后，下一个瓶颈是什么？**——通常是数据搬运：内存层级、occupancy 的边界和 roofline 会告诉你接下来该修什么。这是 Part III–V。
🗣 讲完三个块后补一句（对应页面底部的收束条）：**"前半场回答怎么让 GPU 忙起来；后半场回答 GPU 忙起来以后，为什么还可能不快。"**

📖 页面对照：三个问题块 + 底部收束条 "first half: how to make the GPU busy. Second half: why a busy GPU can still be slow."
...
🎤 这次路线图不是目录，而是**问题清单**——表格右列就是每个部分要回答的问题：**Part I GPU 架构**（4–19 页）回答"并行工作如何被组织和调度"（中间我自己画了两张比喻图帮大家建立直觉）；**Part II CUDA 编程**（20–27）回答"如何把一个计算映射到 GPU 线程上"；**Part III 内存层级**（28–36）回答"数据放在哪里、搬动它的代价是什么"；**Part IV Occupancy 实战**（37–45）回答"更多 warp 什么时候真能提速"；**Part V 正确性与 Roofline**（46–49）回答"下一步该优化计算还是内存"。
🗣 五部分的关系一句话：Part I 讲**硬件怎样执行**并行工作，Part II 讲**程序怎样描述**并行工作，Part III 讲**数据怎样供应**这些计算，Part IV 讲**更多并行度何时还有帮助**，Part V 讲**下一个瓶颈在哪里**。
...
🎤 先定位这一页：前面讲的是并行的**第一层收益**——很多线程同时计算；这一页讲**第二层收益**——某个 warp 暂时动不了时，其他 warp 怎么让 SM 保持忙碌。这才是 latency hiding 的正式出场。让上一页静态的图**动起来**：假设某个分区此刻：Warp A 在等 HBM 数据、Warp B 在等上一条计算结果、Warp C 和 D 数据都齐了。调度员**不会**傻等 A 和 B——它直接发射就绪的 C 或 D，跳过停顿的 warp 不花任何代价。下一拍它可能又挑别的：周期 1 发 C、周期 2 发 F、周期 3 发 D、周期 4 发 H……
...
## 第 15 页｜Occupancy Gives the Scheduler More Choices（occupancy = 给调度员更多选择）⭐
...
## 第 29 页｜Register Pressure Can Turn Fast Storage into DRAM Access（寄存器压力会把最快的存储变成 DRAM 访问）
...
## 第 35 页｜Unified Memory Hides Copies, Not Their Cost（统一内存隐藏的是拷贝，不是拷贝的代价）
...
下面两条规则务必分清：**规则一**——occupancy 低且性能差：第一味药是**加并行度**，加到"有足够多的 ready warp 能把延迟藏住"为止——**性能不再上涨（plateau）或别的瓶颈占主导时就停**，不要以某个固定百分比为目标（第 42 页 38.7% 拿到 22 倍就是证据）；**规则二**——occupancy 已经中高但 kernel 是 memory-bound：推到 100% **没用**——你只需要"刚好够藏延迟"的 warp 数，之后瓶颈在带宽。
...
## 第 42 页｜Parallelism Cuts Runtime 22× at Only 38.7% Occupancy（22 倍提速，只用了 38.7% 占用率）⭐
...
## 第 43 页｜A Busy GPU Can Still Be Waiting on Memory（忙碌的 GPU 仍可能在等内存）⭐
...
## 第 47 页｜Roofline Tells Us Which Resource Is the Limit（roofline 告诉你极限在哪种资源上）⭐建议讲 2–3 分钟
