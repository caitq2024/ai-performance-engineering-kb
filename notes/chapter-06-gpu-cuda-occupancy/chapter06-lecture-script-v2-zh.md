...
**问题一：GPU 怎么隐藏延迟？**——不是靠大缓存，而是靠几千个线程：一个 warp 在等，另一个就顶上。这是 Part I。
...
🎤 这次路线图不是目录，而是**问题清单**——表格右列就是每个部分要回答的问题：**Part I GPU 架构**（4–19 页）回答"GPU 如何隐藏延迟"（中间我自己画了两张比喻图帮大家建立直觉）；**Part II CUDA 编程**（20–27）回答"如何产生足够多的并行工作"；**Part III 内存层级**（28–36）回答"数据在哪里等待"；**Part IV Occupancy 实战**（37–45）回答"更多 warp 什么时候真能提速"；**Part V 正确性与 Roofline**（46–49）回答"下一步该优化计算还是内存"。
...
🗣 **v2 必说（讲完定义马上补，防止第 51 页"反转"显得突然）**："occupancy 不是 GPU 有多忙，也不是性能分数。它只说明 SM 上有多少 resident warps 可供 scheduler 挑选。足够多可以隐藏延迟；达到足够之后，继续提高可能没有收益。"
